...
const RegisterForm: React.FC = () => {
    ...
    const handleGithubLogin = () => {
        ...
        if (!GITHUB_CLIENT_ID) {
            console.error("GitHub Client ID is missing in .env");
            return;
        }

        const redirectUri = window.location.origin + '/profile';
        const scope = "user:email";
        window.location.assign(`https://github.com/login/oauth/authorize?client_id=${GITHUB_CLIENT_ID}&redirect_uri=${redirectUri}&scope=${scope}`);
    };
    ...
    return (
        <div className={`rounded-2xl shadow-xl backdrop-blur-md border p-8 animate-scale-in animation-delay-200 opacity-0 ${darkMode ? 'bg-zinc-900/50 border-white/10' : 'bg-white/80 border-zinc-200'}`}>
            ...
            <div className="grid grid-cols-3 gap-2 animate-fade-in-up animation-delay-700 opacity-0">
                <button onClick={() => googleLogin()} className={`flex items-center justify-center gap-2 py-2.5 rounded-xl border transition-all duration-300 hover:bg-zinc-50 dark:hover:bg-zinc-800 hover:scale-[1.02] active:scale-[0.98] ${darkMode ? 'border-zinc-800 bg-zinc-900 text-white' : 'border-zinc-200 bg-white text-zinc-700'}`}>
                ...
                <button onClick={handleGithubLogin} className={`flex items-center justify-center gap-2 py-2.5 rounded-xl border transition-all duration-300 hover:bg-zinc-50 dark:hover:bg-zinc-800 hover:scale-[1.02] active:scale-[0.98] ${darkMode ? 'border-zinc-800 bg-zinc-900 text-white' : 'border-zinc-200 bg-white text-zinc-700'}`}>
